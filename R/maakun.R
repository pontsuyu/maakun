...
library(animation)
# 2回目以降はコチラ
db <- src_sqlite("maakun.sqlite3", create = F)
...
pitch <- dat %>% filter(pitcher_name==p_name) %>% separate(date, c("year", "month", "day"), sep = "_")
# 球種の割合
pt <- table(pitch$year, pitch$stand, pitch$pitch_type)
pt <- pitch %>% 
